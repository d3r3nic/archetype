...
- The host exposes lifecycle events. The most-used: before a tool call, after a tool call, on stop, on session start, on session end.
- Event data arrives as JSON on stdin. Parse it with a JSON tool if one is available.
- The before-tool-call event is the only one that can block. Exit 2 with a message on stderr to block; the AI sees the stderr and reasons about it.
