...
2. Hierarchy: one primary action per independently understood region, none where read-only; the reading order leads to each action (#31). [finding]
...
One capture per applicable state per scheme per committed context. An applicable state without a capture is not done. The capture set uses the decision basis, artifact revision and code revision above. Revalidate it after any of these inputs change. Component-only states stay with catalog evidence unless they change screen composition. Open every capture and write what it shows: a blank frame, the wrong language, or the wrong state is a finding, not evidence.
