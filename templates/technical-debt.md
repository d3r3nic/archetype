...
- **How the fields are read.** `scripts/validate-profile.sh` reads `Status`, `Kind`, `Control`, `Due-before`, `Review-by` and `Closure-evidence` from each entry, written as a list item or a plain line, with the label bold or not, and the value after the colon. An entry it cannot read, such as one whose Kind is neither shortcut nor deferral, one that mentions deferring with no readable Kind, or a field given two different values, is reported as unverified, which `--strict` fails; it never passes. A deferral with no readable Control or Due-before fails.
