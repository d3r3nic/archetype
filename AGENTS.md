...
The framework's guidance, steps and checks are reminders; the session has the final word on fit, within the decision authority in PROFILE.md. When one does not fit how this application works, or is wrong here, set it aside: record what, why and what you did instead at the decision location, or with `scripts/next-step.sh --set-aside` for a step (development/STEPS.md), and report it upstream when it would get in the way of other projects (development/FEEDBACK.md). The final word is about fit, never about doing the work well. A set-aside is reported as set aside, never as a pass, and it never covers the owner's decisions, the floor and the obligations the project's facts add (#30), or honesty.
