...
- Work that waits on the owner's authorization (opening an account, a recurring cost, accepting terms) is blocked, not deferred: feature-tree.md gives it the status `blocked (owner: <action>)`, its docs/systems/ page names the action and what was built meanwhile (the boundary and a local stand-in), and the request stays open in the owner channel. A deferral is a postponement the operating stage allows (#30); a block is a decision the owner has not made, so it is never filed as a deferral to quiet a gate whose trigger has fired.
