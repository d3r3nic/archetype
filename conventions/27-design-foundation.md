...
- Who picks the look, and how a design tool's output is adopted, is stated once, in "Design tools" below ("Who picks"). Design content follows this discipline; code follows the tokens.
...
Before any screen work a session reads, in order: References.md § Design Artifact, the brand book, the vocabulary, the tokens, the catalog, then the artifact entries for this feature. A screen the artifact covers is built from its entry. A pattern the artifact lacks is sketched first at decision fidelity (a wireframe, or a written composition: purpose, primary action, order, components, states), recorded as session-decided in the entry format of templates/design-artifact-entry.md, then built. The session decides alone: composition within the system, component choice, spacing from the ladder, words in the vocabulary, states and their placement. It asks the owner only when the gap is theirs: identity, what a primary screen is for, a pattern the product would be recognized by. A screen is proved by its capture set before review.
