...
- Will another AI assistant also work on this project, in its own chat, taking turns with me? At each hand-over you would pass one short line from one chat to the other. (Fills `Peer coding` in References.md: none, or both assistants.)
- If so, which of us should write new work, and which should review it? Either of us can fix what a review finds and send it back for another review. Or should we take turns, or will you say each time? (Fills `Peer roles`, in the owner's words.)
