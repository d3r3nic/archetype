...
- `References.md § Design Artifact` for a project with a UI, with every labelled line the matching template carries (`templates/references-frontend.md` or `templates/references-mobile.md`): values from what the audit found, `unknown` where the audit could not establish one, and the direction of truth recorded as a decision (#27, #29). A backend-only project marks the section not applicable in one line. An existing product whose `Brand decided` is not yes runs the design interview (`bootstrap/DESIGN-INTERVIEW.md`) with its owner, with what the audit found already filled in.
- `feature-tree.md` (all systems and features mapped with status)
- In `References.md` § Project, the `Peer coding` line from the owner's Step 2.6 answer: `none`, or `peer-coding/SETTINGS.md`, set up with the engine's `scripts/peer-coding.py setup` and filled with the owner's answers and what the audit verified (`development/PEER-CODING.md`, Setting it up). Step 6's check reads it.
