...
- In `References.md` § Project, the `Peer coding` line from the owner's Step 2.6 answer: `none`, or `peer-coding/SETTINGS.md`, set up with the engine's `scripts/peer-coding.py setup` and filled with the owner's answers and what the audit verified (`development/PEER-CODING.md`, Setting it up). `scripts/validate-migration.sh` checks it.
