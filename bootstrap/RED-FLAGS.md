...
| Real-time + static hosting | Real-time updates need a connection that stays open, which needs a long-lived server. Static hosts cannot. | Add a managed real-time service OR a stateful backend. Confirm the cost addition. |
