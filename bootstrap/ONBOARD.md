...
- **For a PRODUCT: run the design interview** (`bootstrap/DESIGN-INTERVIEW.md`) as part of discovery. It gathers what exists, who uses it and where, what the first screen is for, the owner's own words for things, how it should feel, and the color inputs, and it ends with the direction pick. Its answers fill References.md § Design Artifact (`Primary context`, `Committed contexts`, `Density`, `Vocabulary`) and the brand book. Two of its questions belong here so a short discovery still asks them:
- Should it feel calm, with a few things at a time, or packed with information like a spreadsheet? (Fills `Density`.)
- Is there an app whose feel you like, even if it does something different?
