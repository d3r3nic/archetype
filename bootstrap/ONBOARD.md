...
| References.md § Design Artifact | "Downstream projects decide their own artifact at their own bootstrap. Template ships neutral placeholder tokens — no visual identity invented." `Brand decided: deferred to downstream projects`. | AI researches the tool category, fills every labelled line of the section (direction of truth, sync, working-files folder, `Brand decided` from Group 1) per convention #27. |
