...
A working, empty project with every applicable foundational system in place, or recorded as a deferral with its trigger per the operating stage (#30, `_preamble.md` stage rule) or as blocked on an owner action (#29), plus ONE smoke-test feature that exercises every system that was built. No business features yet. A feature whose systems are in place can be built immediately by plugging into them; one that needs a deferred or blocked system waits for it.
