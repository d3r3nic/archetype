...
This playbook is walked one step at a time (development/STEPS.md). Add `scaffold-frontend` after `bootstrap` on the Playbooks line of PROGRESS.md, then run `scripts/next-step.sh` from the project root: it names the open step, the file that holds it, and what to read for it. Each step after the first is a file of its own; open it when the script names it, not before. A step's conventions are read at that step, whole, and nowhere earlier.
