...
# n/a, plain or in backticks, when the project has no such command. Anything else fails and is never
# run: a sentence such as "set up during scaffold" opens with a shell builtin and would pass if a
# shell read it. read_recorded takes a value that is not empty, bracketed, or a plain none or n/a
# (run_check sorts those out first). It returns 0 with RECORDED set to the command, 1 with CHECK_OUT
# set to why there is none to run, or 2 when the backticks hold only none or n/a.
BT='`'
RECORDED=""
...
read_recorded() { # label, value
  ...
  RECORDED="${rest%%"$BT"*}"
  case "$(trim "$RECORDED" | tr '[:upper:]' '[:lower:]')" in none|n/a) RECORDED=""; return 2 ;; esac
