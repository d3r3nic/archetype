...
- `Skip when:` the one condition under which the step does not apply (`Skip when: the build approach is a platform`), a fact the project records. A step that does not fit how this application works in a way the playbook did not foresee is set aside instead, with the reason (below).
- `Skip by: owner`: optional, for a step that holds the owner's words or guards the whole route (discovery, the build approach, the operating profile, the design interview, the smoke test, the exit gate). Such a step is set aside only on the owner's words.
...
- `--set-aside <id> --reason <text>` records that the session set the step aside: it does not fit how this application works, or its check is wrong for this project. The line says so and that the check did not run; it is never a pass, and it counts like a skip for the steps after it. A step marked `Skip by: owner` also needs `--owner "<the owner's words>"`, which the line keeps. A step that depends on a set-aside step says so when it is named. A closed step whose check no longer fits is reopened first (`--reopen`), then set aside. Name every set-aside step wherever the work is reported, and report the ones that would get in the way of other projects (development/FEEDBACK.md);
...
- `--basis <DEC-NNN>` and `--input <project-relative-path>` may repeat on close, skip or set-aside; a step whose playbook requires a basis records it on each of the three. The tool fingerprints the decision, its transitive decision dependencies and supersession state, plus the exact file content. A Markdown input with `- Decision basis:` must name exactly the supplied decision IDs. Input paths may contain spaces, but not ledger delimiters, control characters, `@sha256:`, absolute paths, tildes, or parent traversal;
- `--list` shows every step with its state: open, closed, skipped, set aside, or reopened.
