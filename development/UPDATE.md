...
Everything the project keeps or carries is written before anything is replaced; if it cannot be written the update stops with nothing replaced.
...
- The framework folders (conventions, backend, frontend, bootstrap, scaffolding, development, templates, scripts) are replaced whole. A file in them that the incoming framework does not ship stops the update before the prompt, named, with nothing written, unless the recorded revision shipped it (the framework retired it), it sits in `conventions/overrides/`, or it is an operating-system or interpreter cache. Move such a file out of the framework folders, or move the project to the engine-folder layout, and run the update again.
