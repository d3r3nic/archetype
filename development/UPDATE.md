...
- `VERSION-LOG.md`: nothing in it moves. Older setup and scaffold instructions put sections after Updates (a second Bootstrap section, a Scaffold section), where later update entries landed. Where update entries (each a dated heading followed by exactly its `Commit`, `Source` and `Updated by: update.sh` lines) end such a section, with only blank lines after them, an `## Updates` heading goes in before them; the preview says `ADD` with the line. Entries a section holds with other lines after them stay as they are, and the preview names them with `NOTE`: they belong under `## Updates`. The new entry goes under `## Updates`, after a new heading when the log still ends in another section, and is written with those headings once the rest of the update has succeeded. A heading inside a fenced block or on an indented line does not count; a fenced block that never closes, a NUL byte in a version log, or a log or project folder that cannot be written stops the update before the prompt, with nothing written.
