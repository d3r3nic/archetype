...
Everything the project keeps or carries is written before anything is replaced; if it cannot be written the update stops with nothing replaced. A listing, hashing, or history step that fails stops the update before the prompt, with nothing written; so does a legacy record (an engine or project-root `VERSION-LOG.md` or `FRAMEWORK-SOURCE.md`) that is a symbolic link.
