...
A feature with a screen reads, in order, `References.md § Design Artifact`, the brand book, the vocabulary, the tokens, the catalog, and the artifact entries for this feature, and names the artifact as a system it will use with the screens and states the artifact covers (the state list in #27). A screen or state the artifact does not cover is sketched first at decision fidelity (a wireframe or a written composition: purpose, primary action, order, components, states), recorded in the artifact as session-decided, and coded after; the owner is asked only when the gap is theirs (#27, #29). Do not improvise a screen because the artifact is silent; that is red flag #10 (see `development/RED-FLAGS.md`).
