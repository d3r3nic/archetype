...
- Artifact-versus-code drift (`References.md § Design Artifact`): the code's tokens equal the tokens source, every implemented screen has its artifact entry, and under `repository-first` the published view is not older than the source (#27)
...
- [ ] Artifact-versus-code drift: tokens in code equal the `Tokens source`, every implemented screen has its artifact entry, and under `repository-first` the published view is not older than the source (#27)
