...
Example: a settings file that names each assistant and the host it runs in, and says which one writes new work and which reviews and fixes what it finds. Hosts differ in which entry file they load (one loads CLAUDE.md, which imports AGENTS.md; another reads AGENTS.md from the repository root down to its working folder) and in what their sandbox allows (one keeps git data read-only and the network off by default, so its commits and pushes ask the owner for approval). Check both for each host when setting peer coding up.
