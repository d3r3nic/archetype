...
- Name yourself in every commit message you write, with a line `Peer: <name>` (the owner's own commits say `Peer: owner`), so the history shows what each assistant did at its turn. `check` fails on a commit without it that is not pushed yet, which you can still reword, and warns on one already pushed, which your packet then attributes; history is never rewritten with a force-push.
